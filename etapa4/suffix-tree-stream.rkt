...
; st-empty? : ST -> Bool
(define st-empty? (lambda(st) (stream-empty? st))
 )

; operatorul care extrage prima ramură a unui ST
; first-branch : ST -> (Label, ST)
(define first-branch (lambda(st) (stream-first st))) ; daca streamul e null
...
(define other-branches (lambda(st) (stream-rest st)))
...
(define (get-ch-branch st ch)
  (cond 
          ...
          ((equal? (car (get-branch-label (first-branch st) ; aici folosesc car sau stream-first????
                         )
                    ) ch  
            ) ;am gasit subtree-ul
