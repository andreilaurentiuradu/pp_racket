...
; st-empty? : ST -> Bool
(define st-empty? (lambda(st) (null? st)
                   )
 )
...
(define (get-ch-branch st ch)
       ;(if (st-empty? st) #f
         ;  (if (get-branch-label (first-branch st)
              ;  );daca e lista
              ; (get-ch-branch (first-branch st) ch); daca e lista facem apel recursiv sa vedem daca primul din lista e tot lista
              ; (if (equal? (get-branch-label (first-branch st)) ch  
      ...
      (cond 
          ...
          ((equal? (car (get-branch-label (first-branch st)
                         )
                    ) ch  
            ) ;am gasit subtree-ul
             (first-branch st) ; luam prima ramura a sa
           ) ;returnam subtree-ul
